...
#coded by Briana
#Now merged with content from Jake's "tktest_vidtest.py"
#to incorporate live feed
...
def spinMotorF():
    #Turns Stepper motor at 1750 pulses/second for 5 seconds (approx. quarter turn clockwise)
    openValve()
    sleep(0.25)
    ticcmd('--exit-safe-start', '--current', '175', '--max-accel', '210000', '--velocity', '17500000')
    sleep(5)
...
switchSpinMotor=Button(cameraGUI, text="FORWARD", command=spinMotorF).place(x=565,y=500)
switchSpinMotor=Button(cameraGUI, text="BACKWARD", command=spinMotorB).place(x=665,y=500)
